...
impl ContributeData {
    pub fn generate_data(svm: &mut LiteSVM, maker: &Keypair, init_data: &InitializeData) -> Self {
        ...
        MintTo::new(svm, maker, &init_data.mint, &contributor_ata, 1_000_000).send();
...
pub(super) fn contribute(
    ...
    let ix_data = [
        ...
        contribute_data.contributing_amount.to_le_bytes().to_vec(),
    ]
    .concat();
    let ix_accounts = [
...
pub fn test_contribute() {
    ...
    let result = contribute(&mut svm, &maker, &init_data, &contribute_data);
    ...
    contribute(&mut svm, &maker, &init_data, &contribute_data).unwrap();
    ...
    msg!("Contribute after start time + duration");
    svm.expire_blockhash();
    let mut clock = svm.get_sysvar::<Clock>();
    let current_time = clock.unix_timestamp;
    clock.unix_timestamp = current_time + init_data.start_time + init_data.duration as i64 + 10i64;
    svm.set_sysvar::<Clock>(&clock);
